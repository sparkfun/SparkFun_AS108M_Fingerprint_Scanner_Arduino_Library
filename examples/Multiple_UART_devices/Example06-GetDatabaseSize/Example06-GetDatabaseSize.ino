/*
  ...
  This example shows how to get the AS-108M/AD-013 database memory size.
...
void loop()
{
  // Get database size from sensor
  uint16_t size = as108m.getDatabaseSize();

  // Send database size to serial port if operation was successful
  if (as108m.response == AS108M_RESPONSE_CODES::AS108M_OK)
  {
    Serial.print(F("Database size is "));
    Serial.print(size);
    Serial.println(F(" entries."));
  }
  else
  {
    Serial.println(F("Error while reading database size."));
  }
