/*
  ...
  This example shows how to get the AS-108M/AD-013 match threshold value.
...
void loop()
{
  // Get match threshold size from sensor
  uint8_t matchThreshold = as108m.getMatchThreshold();

  // Send match threshold to serial port if operation was successful
  if (as108m.response == AS108M_RESPONSE_CODES::AS108M_OK)
  {
    Serial.print(F("Reader match threshold is "));
    Serial.print(matchThreshold);
    Serial.println(F("."));
  }
  else
  {
